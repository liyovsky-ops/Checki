import { deadCodeState, badPatternsState } from './state.js';

let _renderujArchiwum      = () => {};
let _clearDeadCodeHighlights    = () => {};
let _clearBadPatternHighlights  = () => {};

export function initTabs(deps) {
  _renderujArchiwum           = deps.renderujArchiwum;
  _clearDeadCodeHighlights    = deps.clearDeadCodeHighlights;
  _clearBadPatternHighlights  = deps.clearBadPatternHighlights;
}
...
export function switchTab(el, tab) {
  ...
  if (tab === 'nauka') {
    ...
    panel.innerHTML =
      ...
      '<div class="analysis-card" style="cursor:pointer"><div class="card-title">🤖 Agent Backend</div><div class="card-desc">Pisze profesjonalny kod serwerowy — API, bazy danych, autentykacja.</div></div>' +
      '<div class="analysis-card warn" style="cursor:pointer"><div class="card-title">🛡️ Agent Security</div><div class="card-desc">Analizuje kod pod kątem luk bezpieczeństwa i proponuje poprawki.</div></div>' +
      '<div class="analysis-card" style="cursor:pointer"><div class="card-title">🎨 Agent Frontend</div><div class="card-desc">Tworzy komponenty UI, style, responsywny layout.</div></div>';
  } else if (tab === 'archiwum') {
    _renderujArchiwum(panel);
  } else {
    panel.innerHTML =
      '<div style="font-size:12px;color:#94a3b8;margin-bottom:4px;">Podsumowanie kodu:</div>' +
      '<div class="analysis-card ok"><div class="card-title">📄 Co robi ten kod?</div><div class="card-desc">Prosta aplikacja Flask z jednym endpointem <code style="color:#a78bfa">/users</code>. Pobiera użytkowników z bazy danych i zwraca jako JSON.</div></div>' +
      '<div class="analysis-card error"><div class="card-title">💀 Martwy kod</div><div class="card-desc">Funkcja <code style="color:#a78bfa">old_get_users()</code> nigdy nie jest wywoływana.</div><div class="card-line">→ linia 18–22</div></div>' +
      '<div class="analysis-card warn"><div class="card-title">⚠️ Złe rozwiązanie</div><div class="card-desc">Hasło do bazy danych jest wpisane bezpośrednio w kodzie.</div><div class="card-line">→ linia 4</div></div>';
  }
}
...
export function setActive(el) {
  ...
  el.classList.add('active');
  _clearDeadCodeHighlights(); deadCodeState.active = false;
  _clearBadPatternHighlights(); badPatternsState.active = false;
}
...
export function sendAI() {
  ...
  card.innerHTML = '<div class="card-title">💬 Ty: ' + val + '</div><div class="card-desc" style="color:#a78bfa">🤖 AI analizuje... (to demo, AI odpowie po podpięciu backendu)</div>';
  panel.prepend(card);
  input.value = '';
}
